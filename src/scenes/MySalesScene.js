...
class MySalesScene extends Component {
  ...
  updateSalesView(sales) {
    ...
    let statusColors = {
      ...
      Bloqueado: { bg: "#F44336", font: "#FFFFFF" }
    };
        ...
        salesView.push(
          <Card key={i}>
            <View style={styles.cardStyle}>
              <Text>
                Autorização {sale.authorizationCode} em {sale.authorizationDate}{" "}
              </Text>
              <Text
                style={{
              ...
              <View style={styles.saleMessage}>
                <Text style={styles.smallText}>{sale.message}</Text>
              </View>
            </View>
          </Card>
        );
